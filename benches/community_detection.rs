...
use engram::graph::{KnowledgeGraph, GraphNode, GraphEdge};
...
fn generate_graph(node_count: usize, edge_density: f32) -> KnowledgeGraph {
    ...
    let memory_types = vec!["note", "todo", "issue", "decision", "preference", "learning"];
    let tag_pool = vec!["rust", "python", "ai", "database", "web", "api", "cli", "graph", "performance"];
    ...
    for i in 0..node_count {
        ...
        for _ in 0..num_edges {
            let target = rng.gen_range(cluster_start..(cluster_start + cluster_size).min(node_count));
            if target != i {
...
fn bench_detect_communities(c: &mut Criterion) {
    ...
    group.bench_function("detect_communities_500_nodes", |b| {
        b.iter(|| {
            graph.detect_communities(black_box(10))
        })
    });
    group.finish();
}
