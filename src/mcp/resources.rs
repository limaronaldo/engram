...
/// A single resource URI template exposed by engram
#[derive(Debug, Clone)]
pub struct ResourceTemplate {
    /// URI or URI template (e.g. `engram://memory/{id}`)
    pub uri_template: String,
    /// Human-readable name
    pub name: String,
    /// Short description for clients
...
pub fn list_resources() -> Vec<ResourceTemplate> {
    vec![
        ResourceTemplate {
            ...
            description: "Storage statistics across all workspaces".to_string(),
            mime_type: "application/json".to_string(),
        },
        ResourceTemplate {
            uri_template: "engram://entities".to_string(),
            name: "Entities".to_string(),
            description: "Known entities extracted from memories (top 100 by mention count)"
                .to_string(),
            mime_type: "application/json".to_string(),
        },
        ResourceTemplate {
            ...
            description: "A single memory by numeric ID".to_string(),
            mime_type: "application/json".to_string(),
        },
        ResourceTemplate {
            uri_template: "engram://workspace/{name}".to_string(),
            name: "Workspace Statistics".to_string(),
            description: "Statistics for a named workspace".to_string(),
            mime_type: "application/json".to_string(),
        },
        ResourceTemplate {
